...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (usuario.tipo_user === 'VENDEDOR') {
      setConsultorSelecionado(usuario.id_vendedor);
    }
  }, []);
...
// KPIs de quantidade e valor
const totalInteracoes = parceirosFiltrados.reduce((acc, p) => acc + p.total_interacoes, 0);
const totalOportunidades = parceirosFiltrados.reduce((acc, p) => acc + p.total_oportunidades, 0);
const valorTotal = parceirosFiltrados.reduce((acc, p) => acc + Number(p.valor_total_oportunidades || 0), 0);
const totalOrcamentos = parceirosFiltrados.reduce((acc, p) => acc + p.oportunidades_orcamento, 0);
const totalPedidos = parceirosFiltrados.reduce((acc, p) => acc + p.oportunidades_pedido, 0);
...
<Group mt="md" mb="xl" justify="center" grow>
<CardPadrao titulo="Taxa Interação > Oportunidade" valor={`${taxaInteracaoOportunidade.toFixed(1)}%`} cor="#0A5A64" />
<CardPadrao titulo="Taxa Oportunidade > Orçamento" valor={`${taxaOportunidadeOrcamento.toFixed(1)}%`} cor="#0A5A64" />
<CardPadrao titulo="Taxa Orçamento > Pedido" valor={`${taxaOrcamentoPedido.toFixed(1)}%`} cor="#0A5A64" />
</Group>
<Divider style={{ marginTop: 24, marginBottom: 24 }} />
